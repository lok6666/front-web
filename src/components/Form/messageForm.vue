<template>
  <div>
    ...
    >
      ...
      >
        {{ item[i] }}
        <!--输入框-->
        <el-input :readonly="disabled || item.disabled" v-model="item[item.prop]" v-if="item.showInput" :placeholder="item.placeholder" style="width: 284px"/>
        <!--密码-->
 ...
 <!--          <el-tooltip effect="dark" :content="i.label" placement="top-start">
        ...
        <div  v-if="item.showCheckbox" @change="onChangeCheckBox(item, $event)" style="width: 630px;">
          <el-row :gutter="10">
            <el-col v-for="item in item.options" :key="item.id" :span="6">
              <el-checkbox
              :key="index"
              :label="item.label"
              name="type"
        ...
        <!--自定义标签-->
        <div v-if="item.showCustomTag" style="width: 500px;">
          <el-tag
          :key="tag"
